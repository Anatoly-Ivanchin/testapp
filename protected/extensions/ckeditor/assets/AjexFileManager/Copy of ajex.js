var AjexFileManager = 
{
    ...
    open: function (params, url, type, win) 
    {
        ...
        switch (returnTo) 
        {
            case "ckeditor":break;
            case "tinymce": 
                tinyMCE.activeEditor.windowManager.open({url: this.path + "/index.html?type=" + type.toLowerCase() + "&connector=" + this.connector + "&returnTo=" + this.returnTo + "&lang=" + this.lang + "&skin=" + this.skin + "&contextmenu=" + this.contextmenu, width: this.width, height: this.height, inline: "yes", close_previous: "no"}, {window: win, input: params});
                break;
            default:
